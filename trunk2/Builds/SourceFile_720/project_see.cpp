...
project_see::project_see(QString projUUID, QStringList listPicPath,QWidget* parent):
	m_projUUID(projUUID)
	...
	,QWidget(parent)
{
	...
	ui.checkBox_autoRotate->setGeometry(scrWidth - 160 , 30, 100, 20);
	...
	int start = (scrWidth - (item_size * m_listPicPath.size() + 10 * (m_listPicPath.size() - 1) + 20 * 2)) / 2;
	int lenth = (item_size * m_listPicPath.size() + 10 * (m_listPicPath.size() - 1) + 20 * 2);
	if (lenth > scrWidth)
	{
		lenth = scrWidth;
		start = 0;
	}
	ui.listWidget->setGeometry(start, scrHeight - 20 - 64 - 30 - 120, lenth - 10, 120);
	...
	// 场景
	QTimer::singleShot(0, this, &project_see::processNextPicItem);
}

project_see::~project_see()
{}

void project_see::slotClose()
{
	this->close();
	this->destroy(true);
}

void project_see::slotSetSelStyle(QString groupID)
{
	//设置当前选择分组
	if (m_cutGroupID == "")
	{
...
void project_see::Slot_autoRotate(int status)
{
	if (status == 0)
...
void project_see::SlotShowElement(bool bl)
{
	if (bl == false)
	{
...
void project_see::slotFullScreen()
{
	QScreen* screen = QGuiApplication::primaryScreen();
	QRect screenRect = screen->geometry();
	int scrWidth = screenRect.width();
	...
	if (m_isFull == true)
	{
		...
		ui.listWidget->setGeometry((scrWidth - (item_size * m_listPicPath.size() + 10 * (m_listPicPath.size() - 1) + 20 * 2)) / 2, scrHeight - 20 - 64 - 30 - 80 + 5, (item_size * m_listPicPath.size() + 10 * (m_listPicPath.size() - 1) + 20 * 2), 100);
	...
	else
	{
		...
		ui.listWidget->setGeometry((scrWidth - (item_size * m_listPicPath.size() + 10 * (m_listPicPath.size() - 1) + 20 * 2)) / 2, scrHeight - 20 - 64 - 30 - 80 + 5 + getBottomPix(), (item_size * m_listPicPath.size() + 10 * (m_listPicPath.size() - 1) + 20 * 2), 100);
...
void project_see::slotNewPicClick(const QString& picPath)
{
	//变换场景
	m_picPath = picPath;
...
void project_see::Reload()
{
	...
	QTimer::singleShot(0, this, &project_see::processNextHpItem);
}


void project_see::clearListWidgetGroup()
{
...
void project_see::processNextPicItem()
{
	...
	// 继续处理下一个图片，保持UI响应
	QTimer::singleShot(30, this, &project_see::processNextPicItem);
}

void project_see::processNextHpItem()
{
	//加载热点
	if (m_currentHpItemIndex >= m_vecCurPicHp.size())
	{
		//加载界面分组
		QTimer::singleShot(30, this, &project_see::processNextGroupItem);
		return;
	}
	//添加到场景
	...
	QTimer::singleShot(30, this, &project_see::processNextHpItem);
}

void project_see::processNextGroupItem()
{
	//加载分组
