...
public:
    ...
    void setHandleValue(HandlePos pos, double value) 
    {
        ...
        {
            value = qBound(0.1, value, 180.0 - 10.0);
            if(value > m_handleValues[Handle_Bottom_Right])
                value = m_handleValues[Handle_Bottom_Right];
            if (m_handleValues[Handle_Bottom_Right] - value < 10.0)
            {
                m_handleValues[Handle_Bottom_Right] = value + 10.0;
                emit handleValueChanged(Handle_Bottom_Right, m_handleValues[Handle_Bottom_Right]);
            }
        ...
        {
            value = qBound(0.1 + 10.0, value, 180.0);
            if(value < m_handleValues[Handle_Bottom_Left])
                value = m_handleValues[Handle_Bottom_Left];
            if (value - m_handleValues[Handle_Bottom_Left] < 10.0)
            {
                m_handleValues[Handle_Bottom_Left] = value - 10.0;
                emit handleValueChanged(Handle_Bottom_Left, m_handleValues[Handle_Bottom_Left]);
            }
        ...
        if (value < 20.0)
            m_handleValues[Handle_Top] = 20.0;
        else if (value > 120.0)
            m_handleValues[Handle_Top] = 120.0;
...
protected:
    ...
    void paintEvent(QPaintEvent* event) override {
        ...
        // ========== 2. 绘制3个滑块 ==========
        // 2.1 上方滑块（Handle_Top）
        drawHandle(painter, trackCenterY - 8, m_handleValues[Handle_Top],
            QColor(0, 217, 0), Handle_Top); 

        // 2.2 下方左侧滑块（Handle_Bottom_Left）
        drawHandle(painter, trackCenterY + 8, m_handleValues[Handle_Bottom_Left],
            QColor(122, 0, 217), Handle_Bottom_Left); 

        // 2.3 下方右侧滑块（Handle_Bottom_Right）
        drawHandle(painter, trackCenterY + 8, m_handleValues[Handle_Bottom_Right],
            QColor(217, 0, 122), Handle_Bottom_Right); 
    }
    ...
    void mousePressEvent(QMouseEvent* event) override {
        ...
        for (int i = 0; i < 3; ++i) {
            int handleY = (i == Handle_Top) ? (trackCenterY - 8) : (trackCenterY + 8);
